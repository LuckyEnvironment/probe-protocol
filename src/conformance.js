...
export function loadVectors() {
  return readdirSync(vectorsDirectory)
    ...
    .sort()
    .map((name) => JSON.parse(readFileSync(fileURLToPath(new URL(name, vectorsDirectory)), "utf8")));
}
...
/**
 * Run every vector against an implementation.
 *
 ...
 */
export async function runConformance(apply = applyReference) {
  /** @type {CaseResult[]} */
  const results = [];

  for (const file of loadVectors()) {
